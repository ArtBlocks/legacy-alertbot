...
mintQueue.process(
  ...
  async (
    job: { data: { tokenId: string; ownerAddress: string; contract: string } },
    done: () => void
  ) => {
    const { tokenId, ownerAddress, contract } = job.data;
    console.log('[INFO] Processing Token #', tokenId);
    console.log('[INFO] Fetching Complete data for', tokenId);
    const artBlock = await getArtblockInfo(tokenId, contract);
    ...
    const artBlockWithOwner = { ...artBlock, mintedBy, contract };
    ...
    if (process.env.NODE_ENV == 'production') {
      ...
      await delay(5000);
      console.log(
        `[INFO] WOULD have posted ${tokenId} to twitter and discord if prod`
      );
    }
